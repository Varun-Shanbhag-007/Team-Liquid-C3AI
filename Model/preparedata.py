...
def get_simulation_data(county, start_date, end_date):
    population = c3aidatalake.fetch(
        ...
        {
            "spec": {
                "filter": "contains(parent, '" + county + "') && (populationAge == '<18' || populationAge == '18-64' "
                                                          "|| populationAge == '>=65' || populationAge == 'Total') && "
                                                          "gender == 'Male/Female' && year == 2018",
                "limit": -1
            }
        },
        get_all=True
    ...
    for j in range(len(confirmed_recoveries)):
        if confirmed_cases[j] != 0:
            recovery_rate.append(round((confirmed_recoveries[j] / confirmed_cases[j]), 4))
    for j in range(len(confirmed_deaths)):
        if confirmed_cases[j] != 0:
            mortality_rate.append(round((confirmed_deaths[j] / confirmed_cases[j]), 4))

    area_df = pd.read_csv('county_land_area_formatted.csv')
    # county_area = area_df['Land Area'].loc[area_df['Areaname'].replace(" ", "") == county].values[0]
    ...
    prob_visiting_grocery_store = (mobility_trends[county + ".Google_GroceryMobility.data"].values * (
            us_percent_people_visit_grocery_everyday / 100)) / 100
    prob_visiting_restaurant = (mobility_trends[county + ".Google_RetailMobility.data"].values * (
            us_percent_people_visiting_restaurant / 100)) / 100
    prob_visiting_park = (mobility_trends[county + ".Google_ParksMobility.data"].values * (
            us_percent_people_going_to_parks / 100)) / 100

    return confirmed_cases, confirmed_deaths, confirmed_recoveries, infection_rate, mortality_rate, recovery_rate, \
           county_total_pop, prob_visiting_grocery_store, prob_visiting_restaurant, prob_visiting_park
...
def plot_SIR_graph(county, start_date, end_date, plot_susceptible):
    ...
    casecounts = casecounts[
        ['dates', county + '.CDS_Cases.data', county + '.CDS_Recovered.data', county + '.CDS_Deaths.data']]
    casecounts[county + '.Active_Cases.data'] = casecounts[county + '.CDS_Cases.data'] - casecounts[
        county + '.CDS_Recovered.data'] - casecounts[county + '.CDS_Deaths.data']
    casecounts[county + '.Susceptible.data'] = population - casecounts[county + '.CDS_Cases.data'] - casecounts[
        county + '.CDS_Recovered.data'] - casecounts[county + '.CDS_Deaths.data']
